...
export default function PrintLR() {
  ...
  const rows = [
    ...
    ['Bilty Charge', b.biltyCharge],
    ['Door Delivery', b.doorDeliveryCharge],
    ['Insurance', b.insurance],
    ['Loading/Unloading', b.loadingUnloading],
  ...
  return (
    <div className="min-h-screen bg-slate-100 py-6">
      <div className="no-print max-w-4xl mx-auto flex justify-end mb-4 px-4">
        <Button onClick={()=>window.print()} className="bg-[#0B2545] text-white"><Printer className="h-4 w-4 mr-2"/>Print LR</Button>
      </div>
      <div className="max-w-4xl mx-auto bg-white shadow-xl p-8 print:shadow-none print:p-6">
        ...
        <div className="grid grid-cols-2 gap-6 mt-5">
          <Box title="Consignor (Sender)">
            <Row k="Name" v={b.sender?.name}/>
            <Row k="Phone" v={b.sender?.phone}/>
            <Row k="GSTIN" v={b.sender?.gst}/>
            <Row k="Pickup Address" v={b.sender?.address}/>
          </Box>
          <Box title="Consignee (Receiver)">
            <Row k="Name" v={b.receiver?.name}/>
            <Row k="Phone" v={b.receiver?.phone}/>
            <Row k="GSTIN" v={b.receiver?.gst}/>
            <Row k="Delivery Address" v={b.receiver?.address}/>
          </Box>
        </div>

        <div className="grid grid-cols-4 gap-4 mt-4 text-sm">
          <Cell k="Origin" v={b.origin}/>
          <Cell k="Destination" v={b.destination}/>
          <Cell k="Invoice No." v={b.invoiceNumber}/>
          <Cell k="Packages" v={b.packages}/>
          <Cell k="Actual Wt" v={`${b.actualWeight} kg`}/>
          <Cell k="Volumetric Wt" v={`${b.volumetricWeight} kg`}/>
          <Cell k="Chargeable Wt" v={`${b.chargeableWeight} kg`}/>
          <Cell k="Freight Rate" v={`₹${b.freightRate}/kg`}/>
        </div>
